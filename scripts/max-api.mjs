...
export async function resolveChatId(token, { chatId, channelLink }) {
  ...
  if (!cleanLink) {
    throw new Error(
      'Не задан MAX_TARGET_CHAT_ID или MAX_CHANNEL_LINK'
    );
  }

  const response = await fetch(
    `${API_BASE}/chats/${encodeURIComponent(cleanLink)}`,
    {
      headers: {
        Authorization: token
      },
  ...
  if (!response.ok) {
    throw new Error(
      `Не удалось получить chat_id канала: MAX API ${response.status}: ${text.slice(0, 500)}`
    );
  }
  ...
  if (!data.chat_id) {
    throw new Error(
      'MAX не вернул chat_id. Проверьте ссылку канала и добавлен ли бот в канал.'
    );
  }

  if (data.status && data.status !== 'active') {
    throw new Error(
      `Бот не активен в канале: status=${data.status}`
    );
  }
